...
const App: React.FC = () => {
  ...
  const sentinelRef = useRef<HTMLDivElement>(null); // Infinite scroll trigger
  ...
  // ✅ Firestore Pagination: প্রথম batch load
  const fetchMovies = useCallback(async (isFirstLoad = false) => {
    if (isLoadingMore && !isFirstLoad) return;
    setIsLoadingMore(true);
    try {
      let q;
      if (isFirstLoad || !lastDocRef.current) {
        q = query(collection(db, 'movies'), orderBy('createdAt', 'desc'), limit(PAGE_SIZE));
      } else {
        q = query(collection(db, 'movies'), orderBy('createdAt', 'desc'), limit(PAGE_SIZE), startAfter(lastDocRef.current));
      }
      ...
      setMovies(prev => {
        ...
        const unique = newMovies.filter(m => !ids.has(m.id));
        return [...prev, ...unique];
      });
    } catch (error) {
      // Fallback: createdAt field নেই, orderBy ছাড়া try করো
      try {
        let q2;
        if (isFirstLoad || !lastDocRef.current) {
          q2 = query(collection(db, 'movies'), limit(PAGE_SIZE));
        } else {
          q2 = query(collection(db, 'movies'), limit(PAGE_SIZE), startAfter(lastDocRef.current));
        }
        const snap2 = await getDocs(q2);
        if (snap2.empty) { setHasMore(false); setIsLoadingMore(false); return; }
        const fallbackMovies = snap2.docs.map(d => ({ id: d.id, ...d.data() })) as Movie[];
        lastDocRef.current = snap2.docs[snap2.docs.length - 1];
        if (snap2.docs.length < PAGE_SIZE) setHasMore(false);
        setMovies(prev => {
          if (isFirstLoad) return fallbackMovies;
          const ids = new Set(prev.map(m => m.id));
          return [...prev, ...fallbackMovies.filter(m => !ids.has(m.id))];
        });
      } catch(e2) {
        console.warn('Firestore fetch failed:', e2);
      }
    }
    setIsLoadingMore(false);
  }, [isLoadingMore]);
  ...
  // ✅ Infinite Scroll — sentinel div দেখা গেলে পরের batch load হবে
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { rootMargin: '200px' } // ২০০px আগে থেকে trigger হবে
    );
    observer.observe(sentinel);
    return () => observer.disconnect();
  }, [hasMore, isLoadingMore, fetchMovies]);
      ...
      <main className={`px-4 max-w-7xl mx-auto relative z-10 ${activeTab === 'home' ? '-mt-2' : 'pt-20'}`}>
        ...
        {activeTab === 'home' && (
            <>  
                ...
                <div className="sticky top-[60px] z-30 bg-[#0d0d10] -mx-4 px-4 py-4 mb-6 border-b border-white/5 shadow-2xl">
                ...
                <div className="pb-8">
                    ...
                    {isLoadingMore && (
                      <div style={{ display: 'flex', justifyContent: 'center', padding: '20px 0', gap: '6px' }}>
                        {[0,1,2].map(i => (
                          <div key={i} style={{
                    ...
                    {!hasMore && movies.length > PAGE_SIZE && (
                      <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.15)', fontSize: '11px', padding: '12px 0 8px', fontFamily: "'DM Sans', sans-serif" }}>
                        — সব content দেখা হয়ে গেছে —
                      </p>
                    )}
                </div>
            </>
